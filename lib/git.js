...
function commit(path, message) {
    log.log('-- git commit --message="'+ message +'" --\n');
    return run(
        'git',
        'commit',
        '--message="' + message + '"',
...
function push(remoteUrl, pushUrl) {
    var repository = pushUrl || '-u origin master';
    log.log('-- git push ' + repository + ' --\n waiting ..\n');
    return run('git', 'push', repository)
            .then(function(result) { 
                log.log('-- push finished --\n', result.output);
            })
            .catch(function(err) {
